...
export const healthUpdate = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'string',
      group: 'content',
      description: 'Choose what kind of content this is. Articles and Image Posts appear in "Insights for Better Health". Videos and Reels appear in "Videos for General Health".',
      options: {
        list: [
          {title: 'Article — goes to Insights section', value: 'article'},
          {title: 'Image Post — goes to Insights section', value: 'image'},
          {title: 'Video (landscape) — goes to Videos section', value: 'video'},
          {title: 'Reel / Short (portrait 9:16) — goes to Videos section', value: 'reel'},
        ],
        layout: 'radio',
      },
      validation: (r) => r.required(),
    ...
    defineField({
      ...
      type: 'boolean',
      group: 'publishing',
      description: 'Toggle ON to pin this post in the large featured slot on the homepage. Only the most recently published featured post will be shown there. Turn OFF when you want a newer post to take over.',
      initialValue: false,
    }),
    defineField({
      name: 'publishedAt',
